...
class FlashscoreApp(tk.Tk):
    ...
    def _build_team_frame(self, parent, col, team):
        ...
        log1 = tk.Text(log1_container, height=25, width=60)
        ...
        log2 = tk.Text(log2_container, height=25, width=60)
            ...
            def write_team_analysis(team_label, series):
                ...
                for title, ml, blocN in self.motif_configs:
                    ...
                    label_map = {
                        ...
                        "pred_results4":  "4.________SS MAX",
                        "pred_results5":  "5.________SS MIN",
                        "pred_results6":  "6.___SS MAX mult",
                        "pred_results7":  "7.___SS MIN mult",
                        "pred_results12": "12._________TOUT"
                    }
                    ...
                    for key, label in label_map.items():
                        ...
                        ball = "⛔" if count == 0 else round(prediction,3)

                        f.write(f"  {label}, ⚽={ball} | X={count}\n")
                ...
                for title, ml, blocN in self.motif_configs:
                    ...
                    for key, label in label_map.items():
                        ...
                        if count == 0:
                            ...
                            lin = round(results[key]["linear_rebound"],3)
                            peak = round(results[key]["peak_envelope"],3)

                        f.write(
                            f"  {label}, LINEAR ⚽={lin} | PEAK ⚽={peak} | X={count}\n"
                        )
                ...
                f.write("\n📊 GLOBAL LR/PE\n")
                f.write(
                    f"LR ⚽={round(lin['prediction'],3)} | "
                    f"PE ⚽={round(peak['prediction'],3)} | "
                    f"X={lin['count']}\n"
                )
                ...
                f.write(f"events={e}\n")
                f.write(f"positions={p}\n")
                f.write(f"indices={pi}\n")
                f.write(f"matches={m}\n")
                f.write(f"next={next_v}\n")
                f.write(f"linear={reb}\n")
                f.write(f"peak={peak_v}\n")
    ...
    def run_prediction(self, motif_length=2):
        ...
        resA = self.fully_static_method_pattern1(
            seriesA,
            motif_length=ml,
            block_size=blocN,
            motif_next_distance=self.motif_next_distance
        ...
        resC = self.fully_static_method_pattern1(
            seriesC,
            motif_length=ml,
            block_size=blocN,
            motif_next_distance=self.motif_next_distance
        ...
        resA = self.fully_static_method_pattern0(
            seriesA,
            motif_length=ml,
            block_size=blocN,
            motif_next_distance=self.motif_next_distance
        ...
        resC = self.fully_static_method_pattern0(
            seriesC,
            motif_length=ml,
            block_size=blocN,
            motif_next_distance=self.motif_next_distance
        ...
        for title, ml, blocN in self.motif_configs:
            self.write_log(self.log_teamA,  f"===== {title} =====\n")
            ...
            label_map = {
                ...
                "pred_results6":  "6.___SS MAX mult",
                "pred_results7":  "7.___SS MIN mult",
                #"pred_results8":  "8._______(1+2)+4",
                #"pred_results9":  "9._______(1+2)+5",
                #"pred_results10": "10.______(1+2)+6",
                #"pred_results11": "11.______(1+2)+7",
                "pred_results12": "12._________TOUT"
            }
            ...
            for key, label in label_map.items():
                ...
                if count == 0:
                    ...
                    ball_display = round(prediction, 3)
        ...
        # --- fully_static_method_pattern1 ---
        for title, ml, blocN in self.motif_configs:
            self.write_log(self.log_teamC,  f"===== {title} =====\n")
        ...
        for title, ml, blocN in self.motif_configs:
            ...
            for key, label in label_map.items():
                ...
                if count == 0:
                    ...
                    ball_display_lin = f"{round(results[key]['linear_rebound'],3):>6}"
                    ball_display_peak = f"{round(results[key]['peak_envelope'],3):>6}"
                ...
                self.write_log(
                    self.log_teamA,
                    f"  {label_str}, LINEAR ⚽={ball_display_lin} | PEAK ⚽={ball_display_peak} | X={count_str}\n"
                )
        ...
        self.write_log(
            self.log_teamA_1,
            f"LR ⚽={round(linA['prediction'],3)} | PE ⚽={round(peakA['prediction'],3)} | X={linA['count']}\n"
        )
        ...
        self.write_log(
            self.log_teamC_1,
            f"LR ⚽={round(linC['prediction'],3)} | PE ⚽={round(peakC['prediction'],3)} | X={linC['count']}\n"
        )
