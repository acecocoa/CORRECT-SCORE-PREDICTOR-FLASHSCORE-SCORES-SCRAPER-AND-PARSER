...
class FlashscoreApp(tk.Tk):
    ...
    def detect_over_probable(self, series, log_widget):
        data = series[:-1]  # Ignorer le dernier élément
        n = len(data)
        if n < 3:
            return

        # Fenêtres initiales fixes
        windows = [3, 15, 30, 45]
        avg_results = []
        ...
        # Fenêtres successives de 45 éléments après la 45ème dernière
        start = 45
        while start < n:
            end = min(start + 45, n)
        ...
        # Calcul d'une tendance baissière : dérivée simple sur les dernières 10 valeurs
        trend_window = min(10, n)
        recent = data[-trend_window:]
        diffs = [recent[i+1] - recent[i] for i in range(len(recent)-1)]
        trend_score = sum(d for d in diffs if d < 0)  # Somme des baisses
        extreme_fall = trend_score <= -0.5 * trend_window  # seuil ajustable

        # Condition finale : moyenne globale faible OU chute extrême détectée
    ...
    def check_over_probability_hybrid(self, series, log_widget):
        data = series[:-1]  # Exclure le dernier élément
        n = len(data)
        if n < 3:
            return
        ...
        # Fenêtres supplémentaires de 45 éléments après la 45ème
        start = 45
        while start < n:
            end = min(start + 45, n)
            window_data = data[-end:-start] if start != 0 else data[-end:]
    ...
    def _build_ui(self):
        ...
        ttk.Label(top,text="Charger par dossier",font=("TkDefaultFont",10,"bold")).grid(row=0,column=0,sticky="w")

        nav=ttk.Frame(top);nav.grid(row=0,column=1,sticky="e")
        ...
        self.combo_action.set("Charger");self.combo_action.grid(row=0,column=2,padx=4)

        self.combo_scope=ttk.Combobox(controls,values=["Un","Tout"],state="readonly",width=10)
        self.combo_scope.set("Tout");self.combo_scope.grid(row=0,column=3,padx=4)

        ttk.Button(controls,text="Appliquer",command=self.on_apply_clicked).grid(row=0,column=4,padx=6)
    ...
    def _build_log_frame(self,parent,controls):
        ...
        container.rowconfigure(2,weight=1);container.columnconfigure(0,weight=1)

        btns=ttk.Frame(container);btns.grid(row=0,column=0,sticky="ew",pady=(0,4))
        ttk.Button(btns,text="Masquer",command=self.hide_selected_csv).pack(side="left",padx=4)
        ttk.Button(btns,text="Archiver",command=self.archive_selected_csv).pack(side="left",padx=4)

        ttk.Button(container,text="Réinitialiser Log CSV",
    ...
    def apply_csv_action(self):
        ...
        if action=="Charger" and scope=="Tout":pass
        if not display:return
