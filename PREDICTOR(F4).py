...
class FlashscoreApp(tk.Tk):
    ...
    def run_prediction(self, motif_length=2):
        ...
        self.write_log(self.log_teamA,  f"METHODE 1\n")
        for title, ml, blocN in self.motif_configs:
            ...
            for key, label in label_map.items():
                ...
                self.write_log(
                    self.log_teamA,
                    f"  {label}, ⚽={ball_display}\n" # | X={count}
                )

        # --- fully_static_method_pattern1 ---
        self.write_log(self.log_teamC,  f"METHODE 1\n")
        for title, ml, blocN in self.motif_configs:
