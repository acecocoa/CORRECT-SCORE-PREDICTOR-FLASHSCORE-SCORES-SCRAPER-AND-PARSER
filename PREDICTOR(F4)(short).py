...
class FlashscoreApp(tk.Tk):
    ...
    def motif_engine_complete2(self, series, tol=0.15):
        ...
        used_values = set()  # Pour éviter doublons
        used_motifs = set()  # Pour éviter motifs identiques
        ...
        for L in motif_lengths:
            ...
            for label, motif in motifs.items():
                ...
                if motif_tuple in used_motifs:
                    continue  # Ignorer motifs déjà utilisés
    ...
    def decide_divergence_targeted(self, results):
        ...
        # Vérifie s'il existe des prédictions avec correspondances exactes
        exact_predictions = [v['value'] for v in results.values() if v['equal'] > 0]
        if exact_predictions:
            # Priorité au dernier motif exact trouvé
            return exact_predictions[-1]

        # Sinon, vérifie les motifs proportionnels utilisant CR2
        cr2_predictions = [v['value'] for v in results.values() if v['Cr2']]
        if cr2_predictions:
            return cr2_predictions[-1]

        # Si aucune correspondance exacte ou CR2, retourne la première valeur disponible
    ...
    def fully_static_method_pattern1(self, series, motif_length, block_size, motif_next_distance):
        ...
        def check(sim, target):
            return all(v in sim for v in target)
        ...
        for i, sim in enumerate(sim_blocs_list):
            motif_id = tuple(sim)
            if motif_id in used_patterns:
                continue

            if max(sim) > sum(target_motif) / motif_length:
                pr4.append((motif_id, max(sim)))
                used_patterns.add(motif_id)
            elif min(sim) < sum(target_motif) / motif_length:
        ...
        for r in rows:
            ...
            for title in cols:
                ...
                if key not in res:v=""
                    ...
                    entry = res.get(key, {});count = entry.get("count", 0);s     = entry.get("sum", 0)
    ...
    def run_prediction(self,motif_length=2):
        ...
        def write_method3(series,log):
            ...
            res={t:self.fully_static_method_pattern1(series,ml,blocN,self.motif_next_distance)
                 for t,ml,blocN in self.motif_configs2}
        ...
        def write_lower(series,result,log):
            ...
            divergenceT = self.decide_divergence_targeted(results_complete)
