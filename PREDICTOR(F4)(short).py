...
class FlashscoreApp(tk.Tk):
    ...
    def motif_engine_complete2(self, series, tol=0.15):
        ...
        for L in motif_lengths:
            ...
            motifs = {
                ...
                "MIROIR": motif_final[::-1],
                "MIROIR+INV": [-d for d in motif_final[::-1]],
            ...
            for label, motif in motifs.items():
                ...
                for i in range(len(deltas) - L - 1):
                    window = [1 if d > 0 else -1 if d < 0 else 0 for d in deltas[i:i+L]] if label=="SIGNES" else deltas[i:i+L]
                    if window == motif:
                        count_equal += 1
        ...
        if not results:
            results["REPLI"] = {"value": series[-1] + deltas[-1], "equal": 0, "prop": 0, "L": None}
    ...
    def decide_divergence2(self, results):
        ...
        for key in ("MIROIR+INV", "INVERSION", "MIROIR", "DIRECT", "SIGNES", "REPLI"):
    ...
    def run_prediction(self,motif_length=2):
        ...
        def write_lower(series,result,log):
            ...
            for k, v in results_complete.items():
                self.write_log(
                    log,
                    f"⚽={v['value']:<2} | {k:<2} | equal={v['equal']:<2} | prop={v['prop']:<2}\n"
                )
