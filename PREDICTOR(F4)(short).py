...
#===
    def motif_engine_complete2(self, series):
        results = {}
        ...
        for L in motif_lengths:
            ...
            motifs = {
                "DIRECT": motif_final,
                "INVERSION": [-d for d in motif_final],
                "MIROIR": list(reversed(motif_final)),
                "MIROIR+INV": [-d for d in reversed(motif_final)],
                "SIGNES": [1 if d > 0 else -1 if d < 0 else 0 for d in motif_final],
            }
            ...
            for label, motif in motifs.items():
                for i in range(len(deltas) - L - 1):
                    ...
                    if label == "SIGNES":
                        window = [1 if d > 0 else -1 if d < 0 else 0 for d in window]

                    if window == motif:
                        key = f"{label}_L{L}"
                        if key not in results:
                            results[key] = series[i + L + 1]
                        break

        if not results:
            results["REPLI"] = series[-1] + deltas[-1]

        return results

    def decide_divergence2(self, results: dict) -> Optional[int]:
        unique_vals = set(results.values())

        if len(unique_vals) == 1:
            return unique_vals.pop()

        for key in ("MIROIR+INV", "INVERSION", "MIROIR", "DIRECT", "SIGNES"):
    ...
    def motif_engine_targeted_with_cr2(self, series, tol: float = 0.15) -> dict:
        results = {}
        n = len(series)
        ...
        def cr2_difference(next_values, last_value, previous_motif=None):
            first = next_values[0]
            cr_value_numeric = first - last_value

            if previous_motif and previous_motif[0] != 0:
                ratio_disp = first / previous_motif[0]
                ratio_str = f"{ratio_disp:.2f}"
            else:
                ratio_str = f"({first})"

            description = f"Cr=Δ (ratio={ratio_str})"

            if previous_motif:
                delta_r = int(round(first - previous_motif[0]))
                description += f" ΔR={delta_r}"

            return last_value + cr_value_numeric, description
        ...
        for L in (3, 4, 5):
            ...
            for i in range(n - L - 1):
                ...
                if is_proportional(motif_H, motif_C, tol):
            ...
            if equal_next:
                ...
                cr2_used = True
                cr2_value = prediction_value - last_value  
            ...
            results[f"L{L}"] = {
                ...
                "Cr2": cr2_used,
                "cr2_value": cr2_value,       
                "cr2_description": cr2_description,  
    ...
    def detect_target_motif_prediction_disorder(
        self,
        series,
        motif_length,
        block_size,
        ...
        if motif_type is None:
            motif_type = "croissant"

        if len(series) < block_size:
            return {"motif_cible": None, "similar_blocks": [], "prediction": None}

        bloc_cible = series[-block_size:]
        bloc15 = bloc_cible[:-1]
        ...
        similar_blocks = []
        delta_cible = [motif_main[i+1]-motif_main[i] for i in range(len(motif_main)-1)]

        # --- Recherche blocs similaires ---
        for i in range(len(series) - block_size):
            bloc = series[i:i+block_size]
            bloc15_histo = bloc[:-1]

            var_values = bloc15_histo[-motif_length:]
            delta_bloc = [var_values[j+1]-var_values[j] for j in range(len(var_values)-1)]
            ...
            if motif_type == "proportionnel":
                if all(d != 0 for d in delta_cible) and all(d != 0 for d in delta_bloc):
                    ...
                    if max(ratios) - min(ratios) < 1e-6:
                        similar_blocks.append(bloc)
            else:
                if delta_bloc == delta_cible:
                    similar_blocks.append(bloc)

        # --- Nouvelle prédiction ---
        if similar_blocks:
            followers = [bloc[-1] for bloc in similar_blocks]
            n = len(followers)

            # Construction des poids
            if n % 2 == 1:
                center = n // 2
                weights = [i - center for i in range(n)]
            else:
                mid_left = n//2 - 1
                mid_right = n//2
                weights = []
                for i in range(n):
                    if i <= mid_left:
                        weights.append(i - mid_left)
                    else:
                        weights.append(i - mid_right)

            # Calcul évolution pondérée
            weighted_sum = sum(followers[i] * weights[i] for i in range(n))
            evolution = weighted_sum / n

            last_val_motif = motif_main[-1]
            prediction = last_val_motif + evolution
        else:
            prediction = None

        return {
            "motif_cible": motif_main,
...
#===
    ...
    def display_consecutive_stats(self,series,log_widget):
        ...
        self.write_log(log_widget,f"Consecutive most recent: 0={zero_last}* | 1={one_last}*\n")
        ...
        self.write_log(log_widget,f"Consecutive in 15 last: 0={max0}* | 1={max1}*\n")
    ...
    def compute_over_under_full(self, series, log_widget):
        ...
        self.write_log(
            log_widget,
            f"\n📊 {label(count)} | {label(count2)} | {label(countT)} | {last_value}\n"
            f"(O=Over; U=Under; N=Neutral)\n"
        )
    ...
    def log_motif_table(self, log_widget, motif_configs, results):
        # Largeur fixe pour chaque colonne
        col_width = 12

        # Ligne d'en-tête
        header = " " * 3  # espace pour la colonne des labels (C, D, P)
        ...
        log_widget.insert(tk.END, separator + "\n")
    ...
    def run_prediction(self,motif_length=2):
        ...
        def write_lower(series,result,log):
            ...
            #self.write_log(log,f"motif_engine_complete2 results: {results_complete}\n")

            # --- Motif ciblé avec CR2 ---
            self.write_log(log, "\n📊 ACCURACY:\n")
            ...
            if results_targeted:
                ...
                for k, v in results_targeted.items():
                    cr2_val = v.get("cr2_value") or "X"
                    self.write_log(
                        log,
                        f"⚽={v['value']} | {k} | "
                        f"Δ_cr2={cr2_val} | "
                        f"equal={v['equal']} | prop={v['prop']}\n"
                    )

            # --- Décision de divergence ---
            divergence = self.decide_divergence2(results_complete)
            self.write_log(log,f"\nDon't bet {divergence} if it's two time.\n")
            self.write_log(log,f"However ⚽ minimal is: {divergence}.\n")
            ...
            for b in self.compute_blocks_with_gaps(series,2,None,log):
                self.write_log(log,
                    f"LR ⚽={b.get('linear_rebound',0):.3f} | PE ⚽={b.get('peak_envelope',0):.3f}\n"
                    f"\nCORRECT SCORE ⚽= {b.get('average_remaining',0):.3f}\n"
                )

            self.compute_over_under_full(series, log)

            # --- Disorder / Target motif prediction ---
            self.write_log(log, "\n===> GOAL PROB 1 <===\n")
            motif_configs = [            
                ("1°) ML=2 / Bloc=31", 2, 31),
                ("2°) ML=3 / Bloc=61", 3, 61),
