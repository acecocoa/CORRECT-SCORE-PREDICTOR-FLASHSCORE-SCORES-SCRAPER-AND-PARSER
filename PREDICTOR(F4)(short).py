...
class FlashscoreApp(tk.Tk):
    ...
    def run_benchmark(self):
        ...
        with open(filepath, "a", encoding="utf-8") as f:
            ...
            def run_team_block(series, opponent_series, result, label, score):
                log(f"Nombre de but(s) inscrit(s) de l'équipe {label} = {score} but(s)\n")
                ...
                log(f"\n🎯 MODEL A= {self.predict_score_from_seriesA(series, [''])[0]}\n")
                log(f"\n🎯 MODEL B= {self.predict_score_from_seriesB(series, [''])[0]}\n")
                log(f"\n🎯 MODEL C= {self.predict_score_from_seriesC(series, opponent_series)[0]}\n")
                log(f"\n🎯 MODEL D= {self.predict_score_from_seriesD(series)}\n")
                log(f"\n🎯 MODEL E= {self.predict_score_from_seriesE(series)}\n")

                log("\n📊 ACCURACY COMPLETE:\n")
                for k, v in results_complete.items():
                    log(f"⚽={v['value']} | {k} | eq={v['equal']} | pr={v['prop']}\n")

                log(f"\n⚽ could be: {divergenceT}\n")
                ...
                self.display_motifs(
                    series, motif, e, p, pi, m, next_v, reb, peak_v,
                    log, "motif", self.linear_rebound, self.peak_envelope
                )

                log("\n📊 🎯UNDER LINEAR REBOUND🎯\n")
                for b in self.compute_blocks_with_gaps(series, 2, None, log):
                    log(
                        f"LR ⚽={b.get('linear_rebound',0):.3f} | "
                        f"PE ⚽={b.get('peak_envelope',0):.3f}\n"
                        f"LR1/PE1 MEAN ⚽= {b.get('average_remaining',0):.3f}\n"
                    )
    ...
    def decide_final_score(self, series, opponent_series):

        results_complete = self.motif_engine_complete2(series)
        results_targeted = self.motif_engine_targeted_with_cr2(series)

        scores = {}

        def add_score(s, w):
            s = int(round(s))
            scores[s] = scores.get(s, 0) + w

        for v in results_targeted.values():
            weight = v["equal"] * 3 + v["prop"] * 0.5
            add_score(v["value"], weight)

        for v in results_complete.values():
            weight = v["equal"] * 2 + v["prop"] * 0.3
            add_score(v["value"], weight)

        mA, _ = self.predict_score_from_seriesA(series, opponent_series)
        mB, _ = self.predict_score_from_seriesB(series, opponent_series)
        mC, _ = self.predict_score_from_seriesC(series, opponent_series)
        mD = self.predict_score_from_seriesD(series)
        mE = self.predict_score_from_seriesE(series)

        model_values = [
            float(mA), float(mB), float(mC), float(mD), float(mE)
        ]

        model_mean = sum(model_values) / len(model_values)

        add_score(round(model_mean), 5)

        lin = self.linear_rebound_prediction(series)
        peak = self.peak_envelope_linear_prediction(series)

        trend_score = (lin["prediction"] + peak["prediction"]) / 2
        add_score(round(trend_score), 3)

        if not scores:
            return 0

        best_score = max(scores.items(), key=lambda x: x[1])[0]

        last10 = sum(series[-11:-1]) / min(10, len(series)-1)

        if abs(best_score - last10) > 3:
            best_score = round((best_score + last10) / 2)

        return max(0, int(best_score))
    ...
    def run_prediction(self,motif_length=2):
        ...
        def write_lower(series, opponent_series, result, log, team_label):
            ...
            try:
                ...
                self.write_log(log, f"🏁 __FINAL PREDICTION ⚽= {final_score}\n")

            except Exception as e:
                self.write_log(log, f"[ERROR predict_score] {e}\n")

            try:
                final = self.decide_final_score(series, opponent_series)
                self.write_log(log, f"\n🏁 FINAL PREDICTION 2 ⚽= {final}\n")

            except Exception as e:
                self.write_log(log, f"\n[ERROR FINAL PREDICTION 2] {e}\n")
                self.write_log(log, f"\n[ERROR FINAL PREDICTION 2] {e}\n")
            ...
            motif, data = result
            e, p, pi, m, next_v, reb, peak_v = data
            self.display_motifs(series, motif, e, p, pi, m, next_v, reb, peak_v, log,
                                "motif", self.linear_rebound, self.peak_envelope)
