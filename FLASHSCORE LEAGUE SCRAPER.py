...
class FlashscoreGUI(tk.Tk):
    def __init__(self):
        ...
        tk.Button(frame_buttons, text="GET SCORES", command=self.run_scores).grid(row=0, column=1, padx=5)
        ...
        tk.Label(self, text="LEAGUES LIST").pack(anchor="w", padx=10)
        self.list_leagues = tk.Listbox(self, selectmode=tk.MULTIPLE, height=25)
    ...
    def run_scores(self):
        ...
        async def runner():
            async with async_playwright() as p:
                ...
                self.log_msg(f"[INFO] {len(scanned)} duels filtrés")
        ...
        try:
            ...
            self.log_msg(f"[OK] {len(self.scanned_matches)} duels traités")
        except Exception as e:
    ...
    def scan_leagues(self):
        ...
        if self.list_leagues.curselection():
            selected_leagues = [
                self.list_leagues.get(i).rsplit(" (", 1)[0].lower()
                for i in self.list_leagues.curselection()
            ]
        ...
        async def runner():
            async with async_playwright() as p:
                ...
                if leagues_dict:
                    ...
                    self.log_msg(f"[OK] {len(leagues_dict)} ligues détectées automatiquement")
        ...
        if selected_leagues:
            self.scanned_matches = [
                m for m in self.scanned_matches
                if any(lg in (m.get("league") or "").lower() for lg in selected_leagues)
            ]
            self.log_msg(
                f"[LIGUES] Filtrage actif : {', '.join(selected_leagues)} → "
                f"{len(self.scanned_matches)} matchs conservés"
            )
