...
def interpolate_envelope(x_env, y_env, x_full):
    if len(x_env) < 2:
        return np.full_like(x_full, np.mean(y_env) if len(y_env) else 0)
    return np.interp(x_full, x_env, y_env)

def predict_hybrid(y, steps=1):
    ...
    def safe_interp(xe, ye):
        if len(xe) < 2:
            return np.full_like(x, np.mean(y))
        return np.interp(x, xe, ye)

    env_max = safe_interp(x[peaks_max], y[peaks_max])
    env_min = safe_interp(x[peaks_min], y[peaks_min])
...
class FlashscoreApp(tk.Tk):
    ...
    def compute_blocks_with_gaps(self,series,min_block=2,gap_between=None,log_widget=None):
        ...
        if block_avgs:
            lin_val = predict_hybrid(block_avgs)
            peak_val = predict_hybrid(block_avgs)

            lin = {"prediction": lin_val, "count": len(block_avgs)}
            peak = {"prediction": peak_val, "count": len(block_avgs)}
    ...
    def fully_static_method_with_patterns(self,seriesA,seriesC):
        ...
        if motifA:
            ...
            rebA = predict_hybrid(nextA)
            peakA = predict_hybrid(nextA)
        ...
        if motifC:
            ...
            rebC = predict_hybrid(nextC)
            peakC = predict_hybrid(nextC)
    ...
    def display_motifs(
        self,
        series, motif,
        e_list, prop_superieur, prop_inferieur,
        m_list, next_vals, rebounds, peaks,
        target, label_prefix,
        lin_pred=None,
        peak_pred=None
    ):
        ...
        if not next_vals or len(next_vals) < 2:
            ...
            lin = predict_hybrid(next_vals)
            peak = predict_hybrid(next_vals)
    ...
    def run_benchmark(self):
        ...
        filepath = "Benchmark_F4(V2).txt.py"
        ...
        with open(filepath, "a", encoding="utf-8") as f:
            ...
            def run_team_block(series, opponent_series, result, label, score):
                ...
                lin = {"prediction": predict_hybrid(series), "count": len(series)}
                peak = {"prediction": predict_hybrid(series), "count": len(series)}
    ...
    def run_prediction(self,motif_length=2):
        ...
        def write_lower(series, result, log, team_label):
            ...
            try:
                ...
                lin_pred = predict_hybrid(series)
                lin = {"prediction": lin_pred, "count": len(series)}

                peak_pred = predict_hybrid(series)
                peak = {"prediction": peak_pred, "count": len(series)}
