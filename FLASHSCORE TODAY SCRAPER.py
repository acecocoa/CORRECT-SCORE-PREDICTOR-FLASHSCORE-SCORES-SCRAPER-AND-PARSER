...
class FlashscoreGUI(tk.Tk):
    def __init__(self):
        ...
        tk.Label(frame_scores, text="TEAM/ TOTAL/ BOTH").grid(row=0, column=0, sticky="w")
        self.combo_mode = ttk.Combobox(frame_scores, values=["1 - TEAM","2 - TOTAL","3 - TEAM + TOTAL"], width=25, state="readonly")
        ...
        # ratio largeur 5/6 vs 1/6
        frame_main.columnconfigure(0, weight=5, uniform="cols")
        frame_main.columnconfigure(1, weight=1, uniform="cols")
        frame_main.rowconfigure(0, weight=1)

        frame_list = tk.Frame(frame_main)
        frame_list.grid(row=0, column=0, sticky="nsew")

        # Frame du haut : texte + recherche
        frame_top_list = tk.Frame(frame_list)
        frame_top_list.pack(fill="x", pady=(0,3))

        # Colonne 0 → MATCHES FOUND
        tk.Label(frame_top_list, text="MATCHES FOUND => ", font=("Arial", 10, "bold"))\
            .grid(row=0, column=0, sticky="w")

        # Colonne 1 → SEARCH
        tk.Label(frame_top_list, text="SEARCH")\
            .grid(row=0, column=1, padx=(10,2), sticky="w")  # réduire le padding pour rapprocher

        # Colonne 2 → champ de recherche
        self.search_var = tk.StringVar()
        self.search_entry = tk.Entry(frame_top_list, textvariable=self.search_var, width=25)
        self.search_entry.grid(row=0, column=2, sticky="w")
        self.search_var.trace_add("write", self.highlight_matches)

        # Scrollbar et Listbox en dessous
        ...
        self.match_listbox.bind("<<ListboxSelect>>", self.update_user_selection)
        
        frame_log = tk.Frame(frame_main)
        frame_log.grid(row=0, column=1, sticky="nsew")

        self.log = scrolledtext.ScrolledText(frame_log)
    ...
    def highlight_matches(self, *args):
        ...
        for i in self.user_selection:
            self.match_listbox.selection_set(i)

        if query:
            for i in range(self.match_listbox.size()):
                text = self.match_listbox.get(i).lower()
                if query in text:
                    self.match_listbox.selection_set(i)
                    self.match_listbox.see(i)

        self.search_active = False

    def update_user_selection(self, event=None):
        if self.search_active:
            return
        self.user_selection = set(self.match_listbox.curselection())
    ...
    def reset_urls(self):
        ...
        #self.log_msg("[INFO] URLs et liste des matchs réinitialisées")
    ...
    def run_scores(self):
        ...
        if not self.scanned_matches:
            self.log_msg("[ERROR] Aucun match scanné")
            return
        ...
        async def runner_all():
            ...
            async with async_playwright() as p:
                ...
                for idx, m in enumerate(selected_matches, start=1):
                    ...
                    if not url_a or not url_c:
                        self.log_msg(f"[SKIP] Match {idx} URL équipe manquante")
                        continue
                    ...
                    try:
                        ...
                        for s in scores_a:
                            ...
                            scores_a_team.append(
                                s["score_home"] if home == team_a_norm
                                else s["score_away"] if away == team_a_norm
                                else 0
                            )
                        ...
                        for s in scores_c:
                            ...
                            scores_c_team.append(
                                s["score_home"] if home == team_c_norm
                                else s["score_away"] if away == team_c_norm
                                else 0
                            )
        ...
        processed_count = asyncio.run(runner_all())

        self.log_msg(f"Scores des {processed_count} match(s) récupéré(s)")
    ...
    def scan_flashscore(self):
        ...
        for idx, m in enumerate(self.scanned_matches, start=1):
            ...
            if not duel_url.startswith("http"):
                self.log_msg(f"[SKIP] Match {idx} sans URL valide")
                m["url_a"] = None
                m["url_c"] = None
                continue
            ...
            label = f"{idx} | {m.get('time','??:??')} | {duel_url}"
            self.match_listbox.insert(tk.END, label)

        self.log_msg(f"{len(self.scanned_matches)} match(s) disponible(s)")
